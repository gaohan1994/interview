...
export default function useBall(startRef, endRef) {
  /**
   * 创建小球载体
   * @param containerRef
   */
  ...
  const runningToken = useRef(false);
  ...
  useEffect(() => {
    if (startRef.current && endRef.current) {
      ReactDOM.render(
        React.createElement(ball, { ref: innerRef, runTime: 1 }),
  ...
  function running() {
    if (startRef && endRef && innerRef) {
      ...
      if (!runningToken.current) {
        runningToken.current = true;

        // 初始定位
        flyOuterRef.style.display = 'block';
        flyOuterRef.style.left =
          startDot.left + startRef.current.clientWidth / 2 + 'px';
        flyOuterRef.style.top =
          startDot.top + startRef.current.clientHeight / 2 + 'px';
